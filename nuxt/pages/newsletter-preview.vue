<template>
  <div class="px-5 md:px-20 bg-slate-50 pb-20 relative">
    <SectionHeading heading="Newsletter Preview" :inverted="true" />

    <div class="relative">

      <!-- HTML preview popup -->
      <div v-if="showingState" class="absolute z-5 flex flex-column w-full bg-white p-2" style="border:1px solid black; z-index:5; flex-direction: column;">
        <div  v-if="showingHTML">
          ...
          <div>
            <textarea v-html="html" rows="30" cols="50" class="w-full preview-html"></textarea>
          </div>
        </div>
        <div v-if="showingNewsletter">
          ...
          <div class="place-content-center">
            <iframe :srcdoc="html" class="border-2 border-slate-700 w-[1200px] min-h-[500px]"></iframe>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div class="" v-for="(newsletter, index) of newsletterData" :key="index">
          ...
          >
            <div>
              <a @click="showPreview[newsletter.title] ? hide(newsletter) : show(newsletter)" class="cursor-pointer btn">Preview</a> &nbsp; &nbsp; <div style="width:150px; display:inline-block;">Status: <strong :class="{ 'text-green-600': newsletter.status === 'ready', 'text-blue-600': newsletter.status === 'approved', 'text-orange-600': newsletter.status === 'draft', 'text-gray-600': newsletter.status === 'archived' }">{{ newsletter.status }}</strong></div> &nbsp; &nbsp; Title: <strong>{{ newsletter.title }}</strong>
            </div>
          </div>
        </div>
      </div>
...
const styleFontFooter = 'font-size:10px; line-height:15.21px; font-weight:400; color:white;';
const styleFontHeading = 'font-family:' + font + '; font-size:31.5px; line-height:44.1px; color:black;';
const styleFontPackageTitle = 'font-family:' + font + '; font-size:17.5px; line-height:26.62px; color:black; font-weight:400;';
const styleFontPackageLocation = 'font-family:' + font + '; font-size:15.73px; line-height:23.92px; color:black; font-weight:400;';
const styleFontText = 'font-family:' + font + '; font-size:14px; line-height:21.29px; color:black;';

// onMounted(() => {
//  asataImageUrl.value = asataImage.value.url;
// });
...
const renderHtmlStart = (newsletter) => {
  ...
  return '<!DOCTYPE html>' + "\r\n"
    ...
    + '<head>' + "\r\n"
    + '<meta charset="utf-8">' + "\r\n"
    + '<title>' + title + ' | Computravel Newsletter</title>' + "\r\n"
    ...
    + '<meta name="twitter:title" content="' + title + ' | Computravel Package" />' + "\r\n"
    ...
    + '<meta property="og:title" content="' + title + ' | Computravel Package"/>' + "\r\n"
...
const renderHeadStyle = () => {
  return '<style>' + "\r\n"
    ...
    + '.footer-holder { ' + renderHtmlStyleMedia('flex-direction:column;') + 'flex-direction:row; }' + "\r\n"
    + '.footer-left { text-align:left; ' + renderHtmlStyleMedia('text-align:center;') + '}' + "\r\n"
    + '.footer-right { text-align:right; ' + renderHtmlStyleMedia('text-align:center;') + '}' + "\r\n"
    + '.intro-enquire { ' + renderHtmlStyleMedia('display:none;') + '; display:block; height:100px; margin-top:15px; }' + "\r\n"
...
const renderHeader = async (newsletter) => {
  ...
  try {
    ...
    const imageLogo = '<img width="110px" src="' + formatImageUrl(logoImageUrl, 112, 28, 90) + '" alt="Computravel logo" style="background-color:#E6EEEA; padding:14px 25px; border-radius:50px; width:112px; margin:0;" />' + "\r\n";
    ...
    //   + '</div>' + "\r\n"
    //   + '</div>' + "\r\n";
    htmlText += '<table cellspacing="0" cellpadding="0" border="0" height="200px" style="width:100%; height:200px;">' + "\r\n"
      + '<tr>' + "\r\n"
  ...
  return renderHtmlInWrapper(htmlText, 'white', '0', formatImageUrl(newsletter.featured_image, 600, 400));
};

const renderFooter = async () => {
  let logoImage = await getImageByName('logo-transparent');
  let logoImageUrl = logoImage[0].image;
  const imageLogo = '<img width="134px" src="' + formatImageUrl(logoImageUrl, 134, 33, 90) + '" alt="Computravel logo" style="width:134px; height:33px;">';
  const address = '<div><a href="https://www.google.co.za/maps/dir//Computravel,+59+Waterloo+Road,+Cnr.+Waterloo+Road+and+Wolfe+Street,+Wynberg,+7800/@-34.0090973,18.4598109,17z/data=!4m8!4m7!1m0!1m5!1m1!1s0x1dcc42f08a750b6f:0x5cb90b25d3b95c58!2m2!1d18.4620049!2d-34.0091018?hl=en&entry=ttu" target="_blank" style="color:white;">59 Waterloo Road, Wynberg, Cape Town</a></div>' + "\r\n"
    + '<div style="margin-top:0.25rem;">Tel: <a href="tel:+27217620511" style="color:white;">021 762-0511</a></div>';

  let htmlText = '<table border="0" cellpadding="0" cellspacing="0" style="height:92px; margin:0;' + renderHtmlStyleMaxWidth() + '" width="' + newsletterWidth + '" height="92px">' + "\r\n"
    + '<tbody>' + "\r\n"
    + '<tr>' + "\r\n"
    + '<td width="50%" align="left" style="text-align:left;">' + imageLogo + '</td>' + "\r\n"
    + '<td width="50%" align="right" style="' + styleFontFooter + ' text-align:right;">' + address + '</td>' + "\r\n"
    + '</tr>' + "\r\n"
    + '</tbody>' + "\r\n"
    + '</table>' + "\r\n";
  ...
  try {
    ...
    if (typeof newsletter.small_print === 'string') {
      if (newsletter.small_print.length > 0) {
        smallPrint += '<div style="clear: both; ' + styleFontText + ' font-weight:600; padding:0 0 5px 0; text-align:left;">' + newsletter.small_print + '</div>' + "\r\n";
      }
    } else {
      console.warn('[' + functionName + '] small print is not a string: ' + typeof newsletter.small_print);
    }
    let asataImage = await getImageByName('asata-logo-rectangle');
    let asataLogoUrl = asataImage[0].image;
    htmlText += '<table border="0" cellpadding="0" cellspacing="0" width="100%" style="margin:0px; border:0;' + renderHtmlStyleMaxWidth() + '">' + "\r\n"
      + '<tbody>' + "\r\n"
      + '<tr>' + "\r\n"
      + '<td align="center" valign="top" width="100%" style="text-align:left;' + renderHtmlStyleMaxWidth() + '">' + "\r\n"
      + smallPrint
      + '<div style="text-align:center; clear: both; ' + styleFontText + ' font-weight:400;">' + "\r\n"
      + '<img src="' + formatImageUrl(asataLogoUrl, 290, 125) + '" style="display: block; margin: 12px auto 12px auto; min-width:100px; min-height:100px; width:50%; height:auto;" />' + "\r\n"
...
const renderStar = () => {
  return '<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="currentColor" viewBox="0 0 24 24" stroke="currentColor" style="width:16px; height:16px; color:black; padding-left:2px; padding-right:2px;">' + "\r\n"
...
const renderEnquireNow = (title) => {
  ...
  return '<a href="mailto:info@computravel.co.za?subject=' + title.replaceAll(' ', '%20') + '" style="font-size:15.73px; line-height:23.92px; color:white; padding:16px 30px; border-radius:30px; background-color:#ace770; margin:0; text-decoration:none;">enquire now</a>' + "\r\n";
};
...
const renderHtmlHeading = (headingType, title) => {
  ...
  return '<table border="0" cellpadding="0" cellspacing="0" style="padding-bottom: 0px; width:100%;">' + "\r\n"
    + '<tbody>' + "\r\n"
    + '<tr>' + "\r\n"
    + '<td width="100%">' + "\r\n"
    + '<' + headingType + ' style="' + styleFontHeading + ' font-weight:' + weight + '; margin-top:0; margin-bottom:0;">' + title + '</' + headingType + '>' + "\r\n"
    + '</td>' + "\r\n"
    + '</tr>' + "\r\n"
    + '</tbody>' + "\r\n"
    + '</table>' + "\r\n";
...
const renderNewsletterBody = (text) => {
  ...
  return '<div style="text-decoration:none; ' + styleFontText + ' text-align:left; margin-top:15px; margin-bottom:15px;">' + "\r\n"
      + text + "\r\n"
      + '</div>' + "\r\n";
};
...
const renderPackageImage = async (packageData) => {
  ...
  return '<div style="overflow:hidden; width:100%; max-height:' + imageHeight + 'px; height:' + imageHeight + 'px;">' + "\r\n"
    + '<img style="height:' + imageHeight + 'px;" src="' + formatImageUrl(imageCode.url, 290, 0, 65) + '" width="290" height="' + imageHeight + '" />'  + "\r\n"
    + '</div>' + "\r\n"
};
...
const renderPackageDetails = (packageData) => {
  ...
  if (locationListText.length > 0) {
    locationText += '<div style="' + styleFontPackageLocation + ' text-align:center; padding:0 10px;">' + locationListText + '</div>' + "\r\n";
  }
  ...
  if (packageData.star_rating > 0) {
    ...
    starText = '<div style="text-align:center; font-size:15.73px; line-height:23.92px;">' + starText + '</div>' + "\r\n";
  }

  return '<div class="package-details-holder">' + "\r\n"
    + '<div style="color:black !important; font-family:' + font + '; ' + styleFontPackageTitle + ' padding:0 10px;">' + titleAdjusted + '</div>' + "\r\n"
    + starText
    + locationText
    + '<div style="text-decoration:none; font-family:' + font + '; ' + styleFontHeading + ' font-weight:400;">' + formatPrice(packageData.price).replace('.00', '') + '</div>' + "\r\n"
    + '</div>' + "\r\n";
};
...
const renderPackageFeatured = async (newsletterData, packagesData) => {
  ...
  try {
    ...
    const packageData = packagesData[0];
    htmlText += '<!-- Featured package start -->' + "\r\n"
      + renderHtmlHeading('h2', featuredTitle)
      + '<div class="featured-package-holder">' + "\r\n"
      + await renderPackage(packageData, 1, 1)
      + '</div>' + "\r\n"
      + '<div style="' + styleFontText + ' text-align:center;">Offer valid for a limited time only.</div>' + "\r\n"
...
const renderPackage = async (packageData, featured, bottomRow) => {
  ...
  try {
    ...
    return '<!-- single package start -->' + "\r\n"
      + '<div class="single-package-holder '+ featuredClass + '"' + styleText + '>' + "\r\n"
      + '<a target="_blank" style="text-decoration: none;" href="' + formatPackageUrl(packageData) + '">' + "\r\n"
      + '<div class="package '+ featuredClass + '">' + "\r\n"
      + '<div class="package-holder-image">' + "\r\n"
      + await renderPackageImage(packageData)
      + '</div>' + "\r\n"
      + '<div class="package-holder-details '+ featuredClass + '">' + "\r\n"
...
const renderPackages = async (packagesData, countriesData, regionsData, citiesData) => {
  if (packagesData.length === 0) {
    return '<!-- No packages available -->' + "\r\n";
  }
  let htmlText = '<!-- Packages start -->' + "\r\n"
  ...
  for (let index in packagesData) {
    ...
    htmlText += '<td width="50%" class="column ' + (packageNumber % 2 === 0 ? 'left' : 'right') + '" style="' + ((packageNumber % 2 === 0 ? 'text-align:left;' : 'text-align:right;')) + '">'
      + await renderPackage(packagesData[index], 0, packageNumber + 2 >= packagesData.length ? 1 : 0)
      + '</td>';
    packageNumber++;
  }
...
const renderHtmlInWrapper = (htmlText, bgColor, marginY, backgroundImage) => {
  ...
  let backgroundStyle = '';
  if (backgroundImage !== '') {
    backgroundStyle = " background:url('" + backgroundImage + "'); background-size:cover; background-position:center;";
    // inlineStyle = inlineStyle + backgroundStyle;
    bgColorTd = '';
    bgColorStyle = '';
  }
  const sideSpacing = '<td width="52px" style="width:52px; min-width:52px; max-width:52px;"></td>' + "\r\n";
  return '<tr>' + "\r\n"
    + '<td align="center" bgcolor="' + bgColor + '" style="' + inlineStyle + '">' + "\r\n"
    + '<table border="0" cellpadding="0" cellspacing="0" style="display:block; font-family:' + font + '; color:black;' + renderHtmlStyleMaxWidth() + 'margin-top:' + marginY + '; margin-bottom:' + marginY + ';' + backgroundStyle + '">' + "\r\n"
    + '<tbody>' + "\r\n"
    + '<tr>' + "\r\n"
    + sideSpacing
    + '<td align="center"' + bgColorTd + ' style="' + bgColorStyle + 'padding-top:0; padding-bottom:0;' + renderHtmlStyleMaxWidth() + '" valign="top" width="' + newsletterWidth + '">' + "\r\n"
...
const renderSocialMedia = () => {
  let htmlText = '<table border="0" cellpadding="0" cellspacing="0" style="margin:0;' + renderHtmlStyleMaxWidth() + '" width="100%">' + "\r\n"
    + '<tbody>' + "\r\n"
    + '<tr>' + "\r\n"
...
const renderHtml = async (newsletter) => {
  try {
    const newsletterPackages = await getNewsletterPackagesData(newsletter.id, 'newsletters_packages');
    const newsletterPackagesFeatured = await getNewsletterPackagesData(newsletter.id, 'newsletters_featured_packages');
    const packageIds = getNewsletterPackagesIds(newsletterPackages);
    const packageIdsFeatured = getNewsletterPackagesIds(newsletterPackagesFeatured);
    const packagesData = await getNewsletterPackagesByIdList(packageIds);
    const packagesDataFeatured = await getNewsletterPackagesByIdList(packageIdsFeatured);
    // const imageIds = getPackageImageIds(packagesData);
    // const imagesData = await getImagesData(imageIds);

    // Build the newsletter
    html.value = renderHtmlStart(newsletter)
      + await renderHeader(newsletter)
      + renderHtmlIntro(newsletter)
      + await renderPackageFeatured(newsletter, packagesDataFeatured)
      + await renderPackages(packagesData, countriesData, regionsData, citiesData)
      + renderSocialMedia()
...
const renderIcon = (name) => {
  ...
  switch (name) {
    case 'instagram':
      return '<svg style="' + widthHeightColor + '" viewBox="0 0 24 24">' + "\r\n"
    ...
    case 'facebook':
      return '<svg style="' + widthHeightColor + '" viewBox="0 0 24 24">' + "\r\n"
        ...
        + '</svg>' + "\r\n";
    case 'twitter':
      return '<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 455.731 455.731" style="' + widthHeightColor + ' enable-background: new 0 0 455.731 455.731" xml:space="preserve">' + "\r\n"
        + '<g>' + "\r\n"
...
const formatImageUrl = (imageId, width, height, quality) => {
  ...
  return process.env("DIRECTUS_URL") + '/assets/' + imageId + '.jpg?quality=' + quality + '&format=auto&fit=cover' + params;
};
...
const formatPackageUrl = (packageData) => {
  ...
  if (typeof packageData.ct_ref === 'undefined' || typeof packageData.slug === 'undefined') {
    console.warn('[formatPackageUrl] package slug and/or ct_ref is undefined');
    return '';
  }
  return getSiteUrl() + '/packages/' + packageData?.ct_ref + '/' + packageData?.slug;
};
...
const selectFallbackImage = (pckage, destinationData) => {
  ...
  if (typeof pckage !== 'undefined' && typeof destinationData !== 'undefined') {
    if (pckage?.images.length === 0 || pckage?.images.filter((item) => !item?.url).length > 0) {
...
const getPackageImage = async (packageData) => {
  ...
  try {
    ...
    if (typeof imageCode.url === 'string' || typeof imageCode.url === 'undefined') {
